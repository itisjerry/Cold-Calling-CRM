...
import { cn, TEMP_COLORS, STATUS_COLORS, relativeTime, daysSince } from "@/lib/utils";
import { Phone, Flame, Clock, ArrowRight, TrendingUp, Users, Star, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Good {greet}, let's close some deals.</h1>
          <p className="text-sm text-muted-foreground mt-1">{now.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</p>
        </div>
        <Button asChild className="bg-gradient-to-r from-primary to-indigo-500"><Link href="/call-mode"><Phone className="h-4 w-4 mr-1.5" />Open Call Mode</Link></Button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <KPI label="Connect rate" value={`${connectRate}%`} sub="last 7 days" icon={TrendingUp} accent="emerald" />
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Link href="/call-mode" className="text-xs text-primary hover:underline inline-flex items-center gap-1">Open Call Mode <ArrowRight className="h-3 w-3" /></Link>
          </CardHeader>
          <div className="px-2 pb-2">
            {queue.length === 0 ? (
              <p className="p-6 text-center text-sm text-muted-foreground">No leads currently inside their calling window.</p>
            ) : queue.map((l, idx) => (
              <Link key={l.id} href={`/leads/${l.id}`} className="flex items-center justify-between gap-2 px-3 py-2.5 hover:bg-accent rounded-md transition-colors">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={weekChart}>
                ...
                <Bar dataKey="calls" radius={[4, 4, 0, 0]}>
                  {weekChart.map((_, i) => <Cell key={i} fill="hsl(var(--primary))" />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <Card className="lg:col-span-2">
          ...
          <div className="p-4 pt-0 grid grid-cols-3 md:grid-cols-6 gap-2">
            {pipelineSnap.map((p) => (
              <Link key={p.stage} href="/pipeline" className="rounded-md bg-muted/40 p-3 hover:bg-muted transition-colors">
                <div className="text-xs text-muted-foreground">{p.stage}</div>
                <div className="text-xl font-bold mt-1">{p.count}</div>
              </Link>
            ))}
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="px-2 pb-3">
            {projects.length === 0 ? (
              ...
              <Link key={p.id} href="/projects" className="block px-3 py-2 hover:bg-accent rounded-md">
