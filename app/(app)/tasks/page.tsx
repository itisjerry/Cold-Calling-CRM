...
export default function TasksPage() {
  ...
  const overdue = tasks.filter((t) => !t.done && t.due_at && new Date(t.due_at) < startOfDay);
  const today = tasks.filter((t) => !t.done && t.due_at && new Date(t.due_at) >= startOfDay && new Date(t.due_at) < endOfDay);
  const upcoming = tasks.filter((t) => !t.done && t.due_at && new Date(t.due_at) >= endOfDay);
  const noDate = tasks.filter((t) => !t.done && !t.due_at);
  const done = tasks.filter((t) => t.done);

  const create = () => {
    if (!form.title.trim()) return;
    addTask({ title: form.title, due_at: form.due_at || null, priority: form.priority });
    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">
        <Card>
          ...
          <div className="p-2 pt-0 max-h-[500px] overflow-y-auto">{overdue.map((t) => <TaskRow key={t.id} task={t} onToggle={() => updateTask(t.id, { done: !t.done })} onDelete={() => deleteTask(t.id)} />)}{overdue.length === 0 && <p className="p-4 text-xs text-muted-foreground text-center">Nothing overdue 👍</p>}</div>
        </Card>
        <Card>
          ...
          <div className="p-2 pt-0 max-h-[500px] overflow-y-auto">{today.map((t) => <TaskRow key={t.id} task={t} onToggle={() => updateTask(t.id, { done: !t.done })} onDelete={() => deleteTask(t.id)} />)}{today.length === 0 && <p className="p-4 text-xs text-muted-foreground text-center">Nothing due today</p>}</div>
        </Card>
        <Card>
          ...
          <div className="p-2 pt-0 max-h-[500px] overflow-y-auto">{[...upcoming, ...noDate].map((t) => <TaskRow key={t.id} task={t} onToggle={() => updateTask(t.id, { done: !t.done })} onDelete={() => deleteTask(t.id)} />)}</div>
        </Card>
        <Card>
          ...
          <div className="p-2 pt-0 max-h-[500px] overflow-y-auto">{done.slice(0, 30).map((t) => <TaskRow key={t.id} task={t} onToggle={() => updateTask(t.id, { done: !t.done })} onDelete={() => deleteTask(t.id)} />)}</div>
        </Card>
      </div>
